...
import { Building2, Users, Church, Calendar, Pencil } from "lucide-react";
    ...
    <>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{departamento.name}</DialogTitle>
          </DialogHeader>
          <div className="space-y-3 mt-2">
            {departamento.description && (
              <p className="text-sm text-slate-600">{departamento.description}</p>
            )}
            {departamento.church && (
              <div className="flex items-center gap-2 text-sm text-slate-600">
                <Church className="h-4 w-4 text-slate-400" />
                <span>{departamento.church.title}</span>
              </div>
            )}
            <div className="flex items-center gap-2 text-sm text-slate-600">
              <Users className="h-4 w-4 text-slate-400" />
              <span>{departamento._count.persons} personas</span>
            </div>
            <div className="flex items-center gap-2 text-sm text-slate-600">
              <Calendar className="h-4 w-4 text-slate-400" />
              <span>{departamento._count.activities} actividades</span>
            </div>
            <div className="pt-1">
              <Badge variant={departamento.active ? "default" : "secondary"}>
                {departamento.active ? "Activo" : "Inactivo"}
